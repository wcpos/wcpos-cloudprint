...
// /api/status calls: hex(HMAC-SHA256(secret, ts + "." + payload)).
...
// reason to allow it.
func VerifySignature(secret []byte, ts, sig string, payload []byte, now time.Time, skew time.Duration) error {
	...
	if !hmac.Equal([]byte(Sign(secret, ts, payload)), []byte(sig)) {
